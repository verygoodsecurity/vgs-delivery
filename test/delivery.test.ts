jest.mock('../src/waitImage', () => {
  return {
    waitImage: jest.fn((_img, url) => new Promise((resolve, reject) => {
...
describe('delivery', () => {
  ...
  it('works for invalid url but valid img url', async () => {
    try {
      const a = await deliver({ keeperUrl: 'invalidUrl', imgUrls: ['http://invalid', 'http://valid'] });
  ...
  it('fails for invalid url and imgs url', async () => {
    try {
      await deliver({ keeperUrl: 'invalidUrl', imgUrls: ['http://invalid', 'https://invalid'] });
    } catch (e) {
      expect(e).toEqual(e);
    }
  });
